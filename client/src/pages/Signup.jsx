...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/'); // Take them home after successful registration
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-73px)] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-72 h-72 bg-pink-600/20 rounded-full blur-3xl"></div>

      {/* Glassmorphism Form Card */}
      <div className="w-full max-w-md bg-white/10 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10 animate-fade-in">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Create Account</h2>
          <p className="text-gray-400 mt-2">Join the ultimate math duel</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-gray-300 text-sm font-medium mb-1.5 pl-1">Wizard Name</label>
            <input
              type="text"
              name="name"
              required
